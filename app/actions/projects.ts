...
export async function getProjects() {
  let projects = await retryDbQuery(() => db.select().from(projectsTable));
...
export async function deleteProject(projectUuid: string) {
  ...
  const projectCount = await retryDbQuery(() =>
  ...
  await retryDbQuery(() =>
    db.delete(projectsTable).where(eq(projectsTable.uuid, projectUuid))
  );
